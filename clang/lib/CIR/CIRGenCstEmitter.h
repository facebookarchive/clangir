...
public:
  ...
  ConstantEmitter(CIRGenFunction &CGF)
      : CGM(CGF.getCIRGenModule()), CGF(&CGF) {}
  ...
  mlir::Value tryEmitForInitializer(const VarDecl &D);
  mlir::Value tryEmitForInitializer(const Expr *E, LangAS destAddrSpace,
                                    QualType destType);
  ...
  // }
  mlir::Value emitForMemory(mlir::Value C, QualType T) {
  ...
  mlir::Value tryEmitPrivateForVarInit(const VarDecl &D);
  mlir::Value tryEmitPrivate(const Expr *E, QualType T);
  mlir::Value tryEmitPrivateForMemory(const Expr *E, QualType T);

  mlir::Value tryEmitPrivate(const APValue &value, QualType T);
  mlir::Value tryEmitPrivateForMemory(const APValue &value, QualType T);
